...
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID('spPerformSelfDiagnosis'))
BEGIN
    DROP PROCEDURE spPerformSelfDiagnosis
END
...
CREATE PROCEDURE spPerformSelfDiagnosis
	@patientId int,
	@mcv decimal(4,2),
	@mch decimal(4,2),
	@hbf decimal(4,2),
...
BEGIN	
	...
	BEGIN TRY
		...
		IF (SELECT hb FROM @getPatientData) < (SELECT min_hb FROM @ageParameter)
		BEGIN
			IF @mcv < (SELECT min_mcv FROM @ageParameter)
			BEGIN
				IF @mch < (SELECT min_mch FROM @ageParameter)
				BEGIN
					IF @hbf IS NOT NULL AND @hba2 IS NOT NULL
					BEGIN
						IF @hbf > (SELECT max_hbf FROM @ageParameter) AND @hba2 > (SELECT max_hba2 FROM @ageParameter)
						BEGIN
		...
		PRINT 'Self Diagnosis Completed. Is patient thalassemiac? : ' + @isThalassemiac;
	END TRY
	BEGIN CATCH
		IF @@TRANCOUNT > 0
